...
async function addToWishlist(req, res, next) {
  try {
    ...
    const [existing] = await pool.query(
      'SELECT * FROM Wishlist WHERE DeviceToken = ? AND ProductId = ?',
    ...
    await pool.query('INSERT INTO Wishlist (DeviceToken, ProductId) VALUES (?, ?)', [deviceToken, productId]);
...
async function removeFromWishlist(req, res, next) {
  try {
    const { deviceToken, productId } = req.body;
    await pool.query('DELETE FROM Wishlist WHERE DeviceToken = ? AND ProductId = ?', [deviceToken, productId]);
