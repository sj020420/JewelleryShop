...
async function createInquiry(req, res, next) {
  try {
    ...
    const { rows } = await pool.query('SELECT * FROM "InquiryHistory" WHERE "InquiryId" = $1', [inserted[0].InquiryId]);
...
async function updateInquiryStatus(req, res, next) {
  try {
    ...
    const result = await pool.query(
      'UPDATE "InquiryHistory" SET "Status"=$1, "UpdatedAt"=NOW() WHERE "InquiryId"=$2',
      [status, req.params.id]
    );
    if (result.rowCount === 0) throw new ApiError(404, 'Inquiry not found.');

    const { rows } = await pool.query('SELECT * FROM "InquiryHistory" WHERE "InquiryId" = $1', [req.params.id]);
