...
async function login(req, res, next) {
  try {
    ...
    const { rows } = await pool.query(
      'SELECT * FROM "Admin" WHERE "Email" = $1 AND "IsActive" = true',
      [email]
    );
    const admin = rows[0];
...
async function getProfile(req, res, next) {
  try {
    const { rows } = await pool.query(
      'SELECT "AdminId", "FullName", "Email", "Role", "ProfileImage", "CreatedAt" FROM "Admin" WHERE "AdminId" = $1',
      [req.admin.adminId]
    );
...
async function updateProfile(req, res, next) {
  try {
    ...
    if (newPassword) {
      const { rows } = await pool.query('SELECT "PasswordHash" FROM "Admin" WHERE "AdminId" = $1', [req.admin.adminId]);
      const isMatch = await bcrypt.compare(currentPassword || '', rows[0].PasswordHash);
      if (!isMatch) throw new ApiError(400, 'Current password is incorrect.');

      const newHash = await bcrypt.hash(newPassword, 10);
      await pool.query(
        'UPDATE "Admin" SET "FullName" = $1, "PasswordHash" = $2, "UpdatedAt" = NOW() WHERE "AdminId" = $3',
        [fullName, newHash, req.admin.adminId]
      );
    } else {
      await pool.query(
        'UPDATE "Admin" SET "FullName" = $1, "UpdatedAt" = NOW() WHERE "AdminId" = $2',
